...
| Testing  | Jest, Supertest (Nest e2e)                               |
...
After a wipe, recreate schema with `npm run migration:run` in `social-network-app-backend` (or restart the backend container, which migrates on boot).
...
1. **Backend:** type-check, lint, build, migrate Postgres service, unit + e2e tests
